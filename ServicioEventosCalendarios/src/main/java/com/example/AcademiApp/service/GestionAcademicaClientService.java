...
import com.example.AcademiApp.model.dto.AsignaturaExternoDTO;
    ...
    public CursoExternoDTO[] obtenerCursos() {
        try {
            ...
            throw new IllegalArgumentException(
                "No se pudo conectar con el Servicio de Gestión Académica."
            );
        }
    }

    public AsignaturaExternoDTO[] obtenerAsignaturas() {
        try {
            return restTemplate.getForObject(GESTION_ACADEMICA_URL + "/asignaturas", AsignaturaExternoDTO[].class);
        } catch (Exception e) {
            throw new IllegalArgumentException(
                "No se pudo conectar con el Servicio de Gestión Académica."
            );
